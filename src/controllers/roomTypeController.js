...
const { roomTypeSchema, updateRoomTypeSchema } = require("../schemas/room_typeSchema");
...
exports.room_type_create = [
  ...
  async (req, res, next) => {
    try {      
      ...
      const result = await crudOperations.insertNewRoomType(client, dbname, roomType);
...
exports.room_types_read = async (req, res, next) => {
    try {
      ...
      const roomTypesByProperty = await crudOperations.findAllRoomTypesByPropertyId(
        client,
        dbname,
        propertyId
      );
...
exports.room_type_read = async (req, res, next) => {
  try {
    ...
    if (!ObjectId.isValid(capId)) {
      res.status(401);
      throw new Error("The submitted id is not a hexadecimal value generated by MongoDB");
    }
...
exports.room_type_delete = async (req, res, next) => {
  try {
    ...
    if (roomTypeDeletedResult === null) {
      res.status(400);
      throw new Error("The required room type could not be deleted because it does not exist in the Database");
    }
